...
import { RuleInput } from '@/types/rule.types';
...
const RuleList: React.FC = () => {
  ...
  const handleActivate = async (id: string) => {
    try {
      await updateRule(id, { active: true } as RuleInput);
    } catch (err: any) {
  ...
  const handleDeactivate = async (id: string) => {
    try {
      await updateRule(id, { active: false } as RuleInput);
    } catch (err: any) {
  ...
  const handleEvaluateRule = async (id: string) => {
    try {
      ...
      const result = await testRule(id, testData);
      
      // Display the evaluation result
      alert(`Rule evaluation result: ${JSON.stringify(result)}`);
    } catch (err: any) {
  ...
  const ruleTypes = ['all', ...new Set(rules.map(rule => rule.type))];
